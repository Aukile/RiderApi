    ...
    public void render(VertexConsumer buffer, Camera renderInfo, float partialTicks) {
        ...
        for(index = 0; index < this.positions.length - 1; ++index) {
            if (this.positions[index] != null && this.positions[index + 1] != null) {
                ...
                float f2 = this.getV0();
                float f3 = this.getV1();
                buffer.addVertex(vertices2[0].x(), vertices2[0].y(), vertices2[0].z()).setUv(f1, f3).setColor(prevR, prevG, prevB, prevA).setUv2(j & '\uffff', 15728880 >> 16 & '\uffff');
                buffer.addVertex(vertices2[1].x(), vertices2[1].y(), vertices2[1].z()).setUv(f1, f2).setColor(prevR, prevG, prevB, prevA).setUv2(j & '\uffff', 15728880 >> 16 & '\uffff');
                buffer.addVertex(vertices2[2].x(), vertices2[2].y(), vertices2[2].z()).setUv(f, f2).setColor(r, g, b, a).setUv2(j & '\uffff', 15728880 >> 16 & '\uffff');
                buffer.addVertex(vertices2[3].x(), vertices2[3].y(), vertices2[3].z()).setUv(f, f3).setColor(r, g, b, a).setUv2(j & '\uffff', 15728880 >> 16 & '\uffff');
                prevR = r;
                prevG = g;
                prevB = b;
                prevA = a;
